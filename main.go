...
func main() {
	...
	// Load From address set if specified.
	if *fromfile != "" {
		err = loadFromAddresses()
		if err != nil {
			log.Fatal(err)
		}
	}
...
func generateMessage(fake *faker.Faker) enmime.MailBuilder {
	...
	// Use provided From address if available.
	var from string
	if fromaddrs != nil {
		from = fromaddrs[random.Intn(len(fromaddrs))]
	} else {
		from = fake.Email()
	}
...
// loadFromAddresses from specified file, one per line
func loadFromAddresses() error {
